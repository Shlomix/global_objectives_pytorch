...
def train_model(data, use_global_objectives):
    ...
    def true_positive_at_false_positive(scores, labels, target_fpr):
        """Computes true positive - at target false positive rate - over data."""
    ...
    for t in range(TRAIN_ITERATIONS):
        ...
        if t % checkpoint_step == 0:
            ...
            true_positive, false_positive = true_positive_at_false_positive(
                np.dot(data['train_data'], w_) + b_,
                data['train_labels'], TARGET_FPR)

            print('Loss = {} TPR = {}, FPR = {}'.format(loss.data, true_positive, false_positive))
    ...
    true_positive, false_positive = true_positive_at_false_positive(
        ...
        data['eval_labels'], TARGET_FPR)

    return true_positive, false_positive, w_, b_
...
def main(unused_argv):
    ...
    tpr_1, fpr_1, w1, b1 = train_model(experiment_data, use_global_objectives=False)
    print('cross_entropy tpr at requested fpr is {} @ {}'.format(tpr_1, fpr_1))

    tpr_2, fpr_2, w2, b2 = train_model(experiment_data, use_global_objectives=True)
    print('dedicated loss tpr at requested fpr is {} @ {}'.format(tpr_2, fpr_2))
